...
% posns = getUniqueUApositions; %get unique positions %remove when hPos is ready

%%%%%%%% left off here %%%%%%%%%%%%%%%%%%%%%%%%%%
nposns=length(posns); %identify number of unique positions

% posnXYZ=zeros(nposns,3); %remove when hPos is ready
...
% for i=1:nposns %remove when hPos is ready
%     j=posns(i);
...
oldylim=ylim(ha);
% minInt=50;
% xInt=round(diff([mean(oldxlim)-fovwidth, mean(oldxlim)+fovwidth])/5/minInt)*minInt;
% yInt=round(diff([mean(oldylim)-fovheight, mean(oldylim)+fovheight])/5/minInt)*minInt;
