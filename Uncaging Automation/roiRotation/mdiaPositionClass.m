...
%                 + uncageTime * length(strfind([t.action],'Uncaging'));
            ...
            dia.test.elapsedTime = elapsedTime;
            dia.test.endTimes = endTimes;
            cla(hAx);
            for j=1:length(t)
            ...
            legend_handle=legend(hAx,'Imaging','Exclusive Imaging','Uncaging');
            legend_markers = findobj(get(legend_handle, 'Children'), ...
                'Type', 'Patch');
            legend_markers=sort(legend_markers); %hopefully this will keep them in order...
            set(legend_markers(1), 'FaceColor', 'b');
