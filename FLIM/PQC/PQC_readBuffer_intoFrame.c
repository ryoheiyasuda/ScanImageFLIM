...
const double    time_per_unit = 12.4612e-9; //seconds.
...
//             mexPrintf("Repeat %d\n", n_events);
            for (i=0; i<nRecords; i++)
            {
                ...
                if (photon && !first_event)
                {
                    ...
                    if (pixel < n_pixels && delta_t < n_dtime)
                    {
...
//                     mexPrintf("wasted photon\n");
        ...
        tend = clock() - tstart;
//         if (iter % 1000 == 0)
//             mexPrintf("Time: %f, line: %d\n", ((float)tend)/CLOCKS_PER_SEC, n_events);
        ...
        else if (n_events >= n_lines || ((float)tend)/CLOCKS_PER_SEC > 1.0)
            goto stoptttr;
    }
