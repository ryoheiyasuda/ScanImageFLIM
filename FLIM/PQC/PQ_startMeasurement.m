...
if focus && state.acq.msPerLine < 1
    if strcmp(state.spc.acq.SPCdata.HW_Model, 'TimeHarp 260 N')
        state.spc.acq.SPCdata.binning = 4;
    elseif strcmp(state.spc.acq.SPCdata.HW_Model, 'TimeHarp 260 P')
        state.spc.acq.SPCdata.binning = 7;
    end
    update_from_GUI = 0;
elseif state.acq.msPerLine < 1
    state.spc.internal.showRealtimeImage = 0; %Too fast to show images during acquisition.
    update_from_GUI = 1; 
else
    update_from_GUI = 1;    
end
...
spc.stack.stackF = zeros(state.spc.acq.SPCdata.adc_resolution, ...
    state.acq.linesPerFrame*state.spc.acq.SPCdata.n_channels, state.acq.pixelsPerLine, nFramesSave, 'uint8');
